...
void ChangeFOV(float FOV)
{
	...
	gluPerspective(FOV,ratio,0.98f,10000.0f);
...
void GetPixel(int x, int y, RGB &kolory)
{
	glReadPixels(x,windowY-y,1,1,GL_RGB,GL_UNSIGNED_BYTE,&kolory); 
}

void litera2(int jaki_znak,GLfloat gdzie_x,
				   GLfloat gdzie_y,GLfloat rozmiarx,float rozmiary,GLuint textura,int r, int g, int b)
{
	if (jaki_znak<0) return;
...
void piszTXT2(GLfloat x,GLfloat y,GLfloat rozmiarx,float rozmiary,float ods,int r, int g, int b,
				GLuint tekstura,char* control,...)
{
	char buf[1000];
    va_list args;
	...
	while(*str!=0)
	{
		litera2(*str,XXX,y,rozmiarx,rozmiary,tekstura,r,g,b);
		XXX+=ods;//(rozmiarx+(rozmiarx/ods));
		str++;
	}
}
#endif